...
// Find ffmpeg: check ./bin first, then system PATH
function findFfmpeg() {
  const local = join(__dirname, '..', 'bin', isWin ? 'ffmpeg.exe' : 'ffmpeg');
...
function resolveUrls(url, formatSelector) {
  try {
    ...
    throw new Error('Could not resolve video URL: ' + (e?.stderr?.toString() || e?.message || e));
  }
}
...
router.get('/stream', (req, res) => {
  ...
  if (!url) return res.status(400).json({ error: 'URL is required.' });
  if (!existsSync(binPath)) return res.status(500).json({ error: 'yt-dlp not found.' });
  if (!ffmpegPath) return res.status(500).json({ error: 'ffmpeg not found. Add ffmpeg to Render build command: apt-get install -y ffmpeg && npm install' });
  ...
  console.log(`[stream] "${filename}" urls=${urls.length} ffmpeg=${ffmpegPath}`);
  ...
  const ffmpegArgs = urls.length === 1
    ? ['-i', urls[0], '-c', 'copy', '-movflags', 'frag_keyframe+empty_moov+faststart', '-f', 'mp4', 'pipe:1']
    : ['-i', urls[0], '-i', urls[1], '-c:v', 'copy', '-c:a', 'copy', '-movflags', 'frag_keyframe+empty_moov+faststart', '-f', 'mp4', 'pipe:1'];
