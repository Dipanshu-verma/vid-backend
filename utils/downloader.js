...
function proxyArgs() {
  ...
  const host = process.env.PROXY_HOST || 'proxy.webshare.io';
  const port = process.env.PROXY_PORT || '80';
  if (user && pass) {
    return ['--proxy', `http://${user}:${pass}@${host}:${port}`];
  }
  return [];
}
    ...
    for (const attempt of YT_CLIENTS) {
      try {
        const args = [
          ...
          '--no-check-certificate',
          '--skip-download',
          ...buildExtractorArgs(attempt.client),
          '--add-header', `user-agent:${attempt.ua}`,
          ...cookies,
          ...proxy,
        ];
...
export async function getVideoInfo(url) {
  ...
  try {
    ...
    console.warn('[downloader] ✗ yt-dlp failed, trying Invidious...');
    try {
      const result = await getVideoInfoFromInvidious(url);
