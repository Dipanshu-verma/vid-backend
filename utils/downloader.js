...
export async function getVideoInfo(url) {
  ...
  if (platform === 'instagram') {
    ...
    const videoId = match[1];
    endpoint = '/youtube/v3/video/details';
    // Request multiple qualities — API renders them server-side
    params = `?videoId=${videoId}&renderableFormats=360p,480p,720p,1080p&urlAccess=normal&fields=contents,metadata`;
  } else if (platform === 'twitter') {
    endpoint = '/twitter/v3/post/details';
    params = `?url=${encodeURIComponent(url)}&fields=contents,metadata`;
  } else {
