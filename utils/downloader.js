...
async function getVideoInfoFromRapidAPI(url, platform) {
  ...
  const API_HOST = process.env.RAPIDAPI_HOST || 'social-media-video-downloader.p.rapidapi.com';
  if (!API_KEY) throw new Error('RAPIDAPI_KEY not set');
  ...
  if (platform === 'instagram') {
    ...
    params = `?shortcode=${shortcode}&renderableFormats=720p,1080p`;
  } else if (platform === 'facebook') {
    endpoint = '/facebook/v3/post/details';
    params = `?url=${encodeURIComponent(url)}&renderableFormats=720p,1080p`;
    ...
    params = `?videoId=${videoId}&renderableFormats=720p,1080p&urlAccess=normal`;
  } else {
    throw new Error(`Unsupported platform: ${platform}`);
  }

  console.log(`[rapidapi] calling ${endpoint} for ${platform}`);

  const res = await fetch(
    `https://${API_HOST}${endpoint}${params}`,
  ...
  if (!res.ok) throw new Error(`RapidAPI error: ${data.message || res.status}`);
  ...
  // Use renderableVideos first (merged video+audio)
...
export async function getVideoInfo(url) {
  ...
  // Non-YouTube: yt-dlp first, RapidAPI fallback
  if (platform !== 'youtube') {
    try {
      return await getVideoInfoViaYtDlp(url, platform);
    } catch (ytDlpErr) {
      const msg = (ytDlpErr?.message || '').toLowerCase();
      if (msg.includes('private') || msg.includes('removed') || msg.includes('unavailable')) {
        throw ytDlpErr;
      }
      console.warn('[downloader] yt-dlp failed, trying RapidAPI...');
      try {
        const result = await getVideoInfoFromRapidAPI(url, platform);
        console.log('[downloader] ✓ RapidAPI succeeded');
        return result;
      } catch (rapidErr) {
        console.error('[downloader] RapidAPI failed:', rapidErr.message);
        throw ytDlpErr;
      }
    }
  }

  // YouTube: yt-dlp → Invidious/Piped → RapidAPI
  console.log('[downloader] YouTube detected, trying yt-dlp first...');
  ...
  try {
    ...
    console.warn('[downloader] ✗ yt-dlp failed, trying Invidious/Piped...');
    try {
      const result = await getVideoInfoFromInvidious(url);
      console.log(`[downloader] ✓ fallback succeeded via ${result._source}`);
      return result;
    } catch (fallbackErr) {
      console.warn('[downloader] ✗ Invidious/Piped failed, trying RapidAPI...');
      try {
        const result = await getVideoInfoFromRapidAPI(url, platform);
        console.log('[downloader] ✓ RapidAPI succeeded');
        return result;
